...
6. Download all signoffs and run `verify:release-signoff` to prove that every
   ...
   sign-off evidence is tied to the intended target-platform runner. Each
   `githubSha` must match `release-manifest.json.sourceGitSha`. The
...
`release-manifest.json.sourceGitSha`, and TypeScript declaration smoke failures.
...
`githubSha` 与 `release-manifest.json.sourceGitSha` 不一致、TypeScript declaration
smoke 未通过的签核文件。`verify:release-signoff`
...
的 `githubRepository` 以及 `npm-publish-artifact.json.sourceRepository` 还必须匹配
`release-manifest.json.sourceRepository`。
