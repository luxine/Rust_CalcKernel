...
4. Download all binaries into `build/npm-binaries`, pack once with
   ...
   including `sourceGitSha` for the checked-out source commit.
...
7. When publication is intentionally approved, rerun or dispatch the workflow
   ...
   `release-manifest.json.sourceGitSha`, so registry replacement evidence is tied to the protected publish
   ...
   `GITHUB_SHA`, `publishProvenance.githubSha`, `release-signoff.json.sourceGitSha`,
   `release-signoff-summary.json.sourceGitSha`,
   `npm-publish-artifact.json.sourceGitSha`, `npm-publish-result.json.sourceGitSha`,
   and every signed target `githubSha` must all match
   `release-manifest.json.sourceGitSha`.
...
binary formats, architectures, sizes, SHA256 values, and `sourceGitSha` must
match the formal release manifest contract.
...
`githubSha` 都必须匹配 `release-manifest.json.sourceGitSha`。
