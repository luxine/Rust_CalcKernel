...
| C backend | `src/backend/mod.rs` 的 C emission 部分 | C/header 输出 parity、dijkstra C dynamic-library runtime parity、f64 edge fixture C dynamic-library runtime parity、C dynamic-library runtime parity、checked/unchecked ABI checks |
...
| LLVM backend | `src/backend/mod.rs` 的 LLVM 部分 | LLVM IR parity、dijkstra LLVM object/dynamic runtime parity、object/dynamic-library runtime parity、target/clang behavior parity |
