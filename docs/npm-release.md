...
4. Download all binaries into `build/npm-binaries`, pack once with
   ...
   including `sourceGitSha` for the checked-out source commit and
   `sourceRepository` for the GitHub repository that produced the manifest.
5. Download the release manifest and the same tarball on every target platform,
   ...
   missing run id, commit SHA, repository, workflow/job, or runner OS/arch
   evidence before writing a platform sign-off, then uploads
...
6. Download all signoffs and run `verify:release-signoff` to prove that every
   ...
   `githubSha`, `githubRepository`, `githubWorkflow`, `githubJob`, `runnerOs`,
   and `runnerArch` so sign-off evidence is tied to the intended
   target-platform runner and source repository. Each `githubSha` must match
   `release-manifest.json.sourceGitSha`, and each `githubRepository` must match
   `release-manifest.json.sourceRepository`. The
...
7. When publication is intentionally approved, rerun or dispatch the workflow
   ...
   source checkout SHA, target matrix, file mode, and binary format / architecture evidence. After publish, it runs `verify:registry-replacement`
   ...
   runner, and requires `publishProvenance.githubSha` to match
   `release-manifest.json.sourceGitSha` and
   `publishProvenance.githubRepository` to match
   `release-manifest.json.sourceRepository`, so registry replacement evidence
   is tied to the protected publish job that held `NPM_TOKEN`.
   ...
   target GitHub Actions provenance (`ciProvider`, `githubRunId`, `githubSha`,
   `githubRepository`, `runnerOs`, `runnerArch`), and
   `sourceFallback: "disabled"`, pre-publish artifact
   ...
   `publishProvenance`, into one final evidence JSON. The publish job
   `GITHUB_SHA`, `GITHUB_REPOSITORY`, `publishProvenance.githubSha`,
   `publishProvenance.githubRepository`, `release-signoff.json.sourceGitSha`,
   `release-signoff.json.sourceRepository`,
   `release-signoff-summary.json.sourceGitSha`,
   `release-signoff-summary.json.sourceRepository`,
   `npm-publish-artifact.json.sourceGitSha`,
   `npm-publish-artifact.json.sourceRepository`,
   `npm-publish-result.json.sourceGitSha`,
   `npm-publish-result.json.sourceRepository`,
   and every signed target `githubSha` must all match
   `release-manifest.json.sourceGitSha`; `GITHUB_REPOSITORY` and
   `publishProvenance.githubRepository`, every `signedTargets[].githubRepository`,
   and every sign-off source repository field must match
   `release-manifest.json.sourceRepository`.
...
source repository (`sourceRepository`, from `GITHUB_REPOSITORY` in CI or
`local` for local clean-source manifests),
...
GitHub Actions provenance (`ciProvider`, `githubRunId`, `githubRunAttempt`,
`githubSha`, `githubRepository`, `githubWorkflow`, `githubJob`) or
target-matching runner evidence
(`runnerOs` / `runnerArch`), enabled source checkout fallbacks, mismatched
`packagedBinarySha256` values, `githubSha` values that do not match
`release-manifest.json.sourceGitSha`, `githubRepository` values that do not
match `release-manifest.json.sourceRepository`, and TypeScript declaration
smoke failures.
...
`githubRunId`, `githubRunAttempt`, `githubSha`, `githubRepository`,
`githubWorkflow`, `githubJob`, `runnerOs`, and `runnerArch` entries,
...
binary formats, architectures, sizes, SHA256 values, `sourceGitSha`, and
`sourceRepository` must match the formal release manifest contract.
...
registry publish result all refer to the same replacement package version and
source repository, including `release-signoff.json.sourceRepository`,
`release-signoff-summary.json.sourceRepository`, and
`signedTargets[].githubRepository`.
...
`interface`, `type`, `enum`, etc.), function signature compatibility, and class
member surface for constructor/property/method/private/static declarations.
The declaration verifier also compares interface member surface by member name,
readonly/optional flags, and TypeScript structural type compatibility, and enum
member surface by member name and constant value. It also compares type alias
...
`publishProvenance`, `sourceRepository`, and public package identity. It must also validate the formal release manifest
...
values before accepting the final bundle. For GitHub Actions publish evidence,
`publishProvenance.githubRepository` must match
`release-manifest.json.sourceRepository`.
...
manifest 还会记录 `sourceGitSha` 和 `sourceRepository`；前者在 CI 中来自
`GITHUB_SHA`，本地来自 `git rev-parse HEAD`，后者在 CI 中来自
`GITHUB_REPOSITORY`，本地 clean-source manifest 使用 `local`。本地生成
manifest 前必须先确认 clean source git worktree；否则 verifier 会用
`source git worktree must be clean` 失败。
...
的每个测试文件都有 Rust 迁移映射。
随后运行 `npm run verify:declaration-parity` 和
`npm run verify:public-api-parity`，确认 package root declaration exports
与 `dist/src/index.d.ts` 的声明 export 名称、declaration export kind 和
function signature compatibility 对齐，并确认 class member surface（constructor、
property、method、private、static 声明）以及 interface member surface（成员名、
...
run id、commit SHA、repository、workflow/job 或 runner OS/arch 的不完整 provenance。
...
GitHub Actions provenance（`ciProvider`、`githubRunId`、`githubRunAttempt`、
`githubSha`、`githubRepository`、`githubWorkflow`、`githubJob`）缺失、runner evidence
（`runnerOs` / `runnerArch`）与目标平台不匹配、
`packagedBinarySha256` 与 release manifest target SHA256 不一致，以及
`githubSha` 与 `release-manifest.json.sourceGitSha` 不一致、
`githubRepository` 与 `release-manifest.json.sourceRepository` 不一致、
...
`packagedBinarySha256`、`nodeVersion`、`npmVersion`、`ciProvider`、
`githubRunId`、`githubRunAttempt`、`githubSha`、`githubRepository`、
`githubWorkflow`、`githubJob`、`runnerOs`、`runnerArch` 证据。最终 cutover evidence 必须归档 signed target
...
manifest 带有正式 release verifier 产生的 metadata、文件面、`sourceGitSha`、
`sourceRepository`、目标矩阵和 binary
...
在 GitHub Actions 中，它还要求 publish job 的 `GITHUB_SHA` /
`publishProvenance.githubSha` 与 `release-manifest.json.sourceGitSha` 一致，
并要求 `GITHUB_REPOSITORY` / `publishProvenance.githubRepository` 与
`release-manifest.json.sourceRepository` 一致。
...
和发布后 registry 结果绑定成同一份最终证据；`release-signoff.json`、
`release-signoff-summary.json`、`npm-publish-artifact.json`、
`npm-publish-result.json`、publish provenance 以及每个 signed target 的
`githubSha` 都必须匹配 `release-manifest.json.sourceGitSha`，publish provenance
的 `githubRepository`、每个 `signedTargets[].githubRepository`、
`release-signoff.json.sourceRepository`、`release-signoff-summary.json.sourceRepository`
以及 `npm-publish-artifact.json.sourceRepository` 还必须匹配
`release-manifest.json.sourceRepository`。
