...
7. When publication is intentionally approved, rerun or dispatch the workflow
   ...
   `npm-publish.json`, and `npm-registry-replacement.json` files alongside
   those verifier outputs so the downloadable cutover bundle is self-contained.
...
`release-manifest/release-manifest.json`、`release/release-signoff.json`、
`npm-publish.json` 和 `npm-registry-replacement.json`，确保下载到的 cutover bundle 可独立追溯。
