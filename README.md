...
- WASM runtime interop，覆盖官方 scalar、calls、control-flow、memory、short-circuit，以及 pricing AoS、f64-array、f64-axpy、f64-sum、pricing SoA 和 TS perf fixture 的 Node 宿主调用结果。
...
- 最终发布切换必须由 Rust package 自身的 release checklist、release verifier、sign-off verifier 和
  ...
  `publish=true`、`NPM_TOKEN` 和 npm provenance 发布路径；
