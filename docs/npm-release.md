...
`dist/src/cli.js` is built and runnable, and the fixture roots used by the Rust
oracle tests are present.
...
`npm run verify:typescript-oracle`；它会只读确认 TypeScript checkout 存在、
`dist/src/cli.js` 已构建且可启动，并确认 Rust oracle tests 依赖的 fixture
目录存在。
